...
@section('content')
<x-common.page-breadcrumb pageTitle="Master Teknisi" />
...
<div x-data="{ showDeleteModal: false, deleteUrl: '' }" class="space-y-6">
    ...
    <div class="flex justify-between items-center">
        ...
        <a href="{{ route('technicians.create') }}" class="inline-flex items-center justify-center rounded-lg bg-brand-500 px-5 py-2.5 text-center text-sm font-medium text-white hover:bg-brand-600 transition">
            + Tambah Teknisi
        </a>
    </div>

    <x-common.component-card title="Daftar Teknisi Internal & Lepas">
        <div class="overflow-hidden rounded-xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
            <div class="max-w-full overflow-x-auto custom-scrollbar">
                <table class="w-full min-w-[800px]">
                    <thead>
                        <tr class="border-b border-gray-100 dark:border-gray-800">
                            <th class="px-5 py-3 text-left sm:px-6"><p class="font-medium text-gray-500 text-theme-xs dark:text-gray-400">Nama Teknisi</p></th>
                            <th class="px-5 py-3 text-left sm:px-6"><p class="font-medium text-gray-500 text-theme-xs dark:text-gray-400">No. HP (WA)</p></th>
                            <th class="px-5 py-3 text-left sm:px-6"><p class="font-medium text-gray-500 text-theme-xs dark:text-gray-400">Spesialisasi</p></th>
                    ...
                    <tbody>
                        ...
                        <tr class="border-b border-gray-100 dark:border-gray-800">
                            <td class="px-5 py-4 sm:px-6">
                                <span class="block font-medium text-gray-800 text-theme-sm dark:text-white/90">{{ $tech->nama_teknisi }}</span>
                            </td>
                            <td class="px-5 py-4 sm:px-6">
                                <span class="block font-medium text-gray-800 text-theme-sm dark:text-white/90">{{ $tech->no_hp ?? '-' }}</span>
                            </td>
                            <td class="px-5 py-4 sm:px-6">
                                <p class="text-theme-xs inline-block rounded-full bg-blue-50 text-blue-700 dark:bg-blue-500/15 dark:text-blue-500 px-2 py-0.5 font-medium">
                                    {{ $tech->spesialisasi ?? 'Umum' }}
                                </p>
                            </td>
                            <td class="px-5 py-4 sm:px-6 text-right">
                                <div class="flex items-center justify-end gap-3">
                                    <a href="{{ route('technicians.edit', $tech->id) }}" class="text-gray-500 hover:text-brand-500 transition-colors dark:text-gray-400 dark:hover:text-brand-500" title="Edit">
                                    ...
                                    <button type="button" @click.prevent="deleteUrl = '{{ route('technicians.destroy', $tech->id) }}'; showDeleteModal = true" class="text-gray-500 hover:text-red-500 transition-colors dark:text-gray-400 dark:hover:text-red-500" title="Hapus">
                        ...
                        <tr class="border-b border-gray-100 dark:border-gray-800">
                            <td colspan="4" class="px-5 py-4 sm:px-6 text-center">
                                <p class="text-gray-500 text-theme-sm dark:text-gray-400">Belum ada data teknisi.</p>
                            </td>
                        </tr>
                        @endforelse
                    </tbody>
    ...
    <div x-show="showDeleteModal" style="display: none;" class="fixed inset-0 z-[99999] flex items-center justify-center bg-black/90 px-4 py-5" x-transition.opacity>
        <div @click.outside="showDeleteModal = false" class="w-full max-w-md rounded-xl bg-white px-8 py-10 text-center shadow-lg dark:bg-gray-900 border border-gray-200 dark:border-gray-800">
            ...
            <h3 class="mt-4 pb-2 text-xl font-bold text-gray-900 dark:text-white">Hapus Data Teknisi?</h3>
            <p class="mb-8 text-sm text-gray-500 dark:text-gray-400">Apakah Anda yakin? Data histori nama teknisi pada layanan servis yang pernah dikerjakan akan dikosongkan.</p>
