...
@section('content')
<x-common.page-breadcrumb pageTitle="Barang Masuk (Restok)" />
...
<div x-data="{ showDeleteModal: false, deleteUrl: '' }" class="space-y-6">
    ...
    <div class="flex justify-between items-center">
        ...
        <a href="{{ route('inbounds.create') }}" class="inline-flex items-center justify-center rounded-lg bg-brand-500 px-5 py-2.5 text-center text-sm font-medium text-white hover:bg-brand-600 transition">
            + Catat Barang Masuk
        </a>
    </div>

    <x-common.component-card title="Riwayat Barang Masuk">
                ...
                <table class="w-full min-w-[800px]">
                    <thead>
                        <tr class="border-b border-gray-100 dark:border-gray-800">
                            ...
                            <th class="px-5 py-3 text-left sm:px-6"><p class="font-medium text-gray-500 text-theme-xs dark:text-gray-400">Unit AC</p></th>
                            <th class="px-5 py-3 text-left sm:px-6"><p class="font-medium text-gray-500 text-theme-xs dark:text-gray-400">Jumlah</p></th>
                            <th class="px-5 py-3 text-left sm:px-6"><p class="font-medium text-gray-500 text-theme-xs dark:text-gray-400">Keterangan / Supplier</p></th>
                    ...
                    <tbody>
                        ...
                        <tr class="border-b border-gray-100 dark:border-gray-800">
                            <td class="px-5 py-4 sm:px-6">
                                <span class="block font-medium text-gray-800 text-theme-sm dark:text-white/90">
                                    {{ \Carbon\Carbon::parse($inbound->tanggal_masuk)->format('d M Y') }}
                                </span>
                            </td>
                            <td class="px-5 py-4 sm:px-6">
                                <span class="block font-medium text-gray-800 text-theme-sm dark:text-white/90">{{ $inbound->item->merek }} {{ $inbound->item->tipe_ac }}</span>
                                <span class="block text-gray-500 text-theme-xs dark:text-gray-400">Kode: {{ $inbound->item->kode_barang }}</span>
                            </td>
                            <td class="px-5 py-4 sm:px-6">
                                <p class="text-theme-xs inline-block rounded-full bg-blue-50 text-blue-700 dark:bg-blue-500/15 dark:text-blue-500 px-2 py-0.5 font-medium">
                                    +{{ $inbound->jumlah_masuk }} Unit
                                </p>
                            </td>
                            <td class="px-5 py-4 sm:px-6">
                                <span class="block text-gray-500 text-theme-sm dark:text-gray-400">{{ $inbound->keterangan ?? '-' }}</span>
                            </td>
                            <td class="px-5 py-4 sm:px-6 text-right">
                                <button type="button" @click.prevent="deleteUrl = '{{ route('inbounds.destroy', $inbound->id) }}'; showDeleteModal = true" class="text-gray-500 hover:text-red-500 transition-colors dark:text-gray-400 dark:hover:text-red-500" title="Batal & Hapus">
                        ...
                        <tr class="border-b border-gray-100 dark:border-gray-800">
                            <td colspan="5" class="px-5 py-4 sm:px-6 text-center">
                                <p class="text-gray-500 text-theme-sm dark:text-gray-400">Belum ada riwayat barang masuk.</p>
                            </td>
                        </tr>
                        @endforelse
                    </tbody>
    ...
    <div x-show="showDeleteModal" style="display: none;" class="fixed inset-0 z-[99999] flex items-center justify-center bg-black/90 px-4 py-5" x-transition.opacity>
        <div @click.outside="showDeleteModal = false" class="w-full max-w-md rounded-xl bg-white px-8 py-10 text-center shadow-lg dark:bg-gray-900 border border-gray-200 dark:border-gray-800">
            ...
            <h3 class="mt-4 pb-2 text-xl font-bold text-gray-900 dark:text-white">Batalkan Barang Masuk?</h3>
            <p class="mb-8 text-sm text-gray-500 dark:text-gray-400">Menghapus riwayat ini akan secara otomatis <b>mengurangi kembali stok AC</b> yang sebelumnya ditambahkan.</p>
