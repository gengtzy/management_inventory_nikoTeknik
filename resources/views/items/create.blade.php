    ...
    <form action="{{ route('items.store') }}" method="POST">
        ...
        <x-common.component-card title="Formulir Data AC">
            <div class="p-5 sm:p-6 grid grid-cols-1 gap-6 xl:grid-cols-2">
                ...
                <!-- KODE BARANG (READONLY OTOMATIS) -->
                <div>
                    <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">
                        Kode Barang <span class="text-xs text-gray-500">(Otomatis)</span>
                    </label>
                    <input type="text" name="kode_barang" value="{{ $nextCode }}" readonly required
                        class="bg-gray-100 cursor-not-allowed dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-11 w-full rounded-lg border border-gray-300 px-4 py-2.5 text-sm text-gray-800 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:text-white/90" />
                </div>
